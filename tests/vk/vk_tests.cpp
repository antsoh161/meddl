...
class MeddlFixture : public ::testing::Test {
  public:
   ...
   void init_device()
   {
      ...
      _device = std::make_unique<NewDevice>(
          physical_device.get(), configs, extensions, std::nullopt, _debugger);
   }
   void init_swapchain()
   {
      SwapchainOptions options{};
      _swapchain = std::make_unique<NewSwapchain>(_instance->get_physical_devices().front().get(),
                                                  _device.get(),
                                                  _surface.get(),
                                                  options,
   ...
   std::unique_ptr<NewDevice> _device{};
   std::unique_ptr<NewSwapchain> _swapchain{};
};
