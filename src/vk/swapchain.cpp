...
//! Helper functions
namespace {
SwapchainDetails get_details(NewPhysicalDevice* device, Surface* surface)
{
...
//   _active_swapchain_info(swapchain_info)
{
   ...
   for (size_t i = 0; i < _images.size(); i++) {
      ...
      if (vkCreateImageView(logical_device, &image_view_info, nullptr, &_image_views[i]) !=
          VK_SUCCESS) {
...
//! New
NewSwapchain::NewSwapchain(NewPhysicalDevice* physical_device,
                           NewDevice* device,
                           Surface* surface,
...
std::vector<VkImageView>& NewSwapchain::get_image_views()
{
   return _image_views;
}
}  // namespace meddl::vk
